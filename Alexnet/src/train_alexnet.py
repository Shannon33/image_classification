...
if __name__ == "__main__":
    ...
    train_transform = transforms.Compose([
        transforms.Resize((256)),      # (256, 256) 区别 只输入256是实现了论文中的缩放256
    ...
    valid_transform = transforms.Compose([
        ...
        transforms.TenCrop(224, vertical_flip=False),       #一张图-->十张图片
        #注意下面这行代码 torch.stack将十张图片拼起来改变bchw中的b  生成5d张量？
        transforms.Lambda(lambda crops: torch.stack([normalizes(transforms.ToTensor()(crop)) for crop in crops])),
    ])

    # 构建MyDataset实例  设置mode 不同模式下
    ...
    num_ftrs = alexnet_model.classifier._modules["6"].in_features   #取出classifier中的第六个网络层 0、1、2、3……
    alexnet_model.classifier._modules["6"] = nn.Linear(num_ftrs, num_classes)       #输入类别，输出类别数
        ...
        if (epoch+1) % val_interval == 0:
            ...
            with torch.no_grad():
                for j, data in enumerate(valid_loader):
                    ...
                    outputs_avg = outputs.view(bs, ncrops, -1).mean(1)  #取平均，固定batchsize ncrops，在ncrops维度取均值
